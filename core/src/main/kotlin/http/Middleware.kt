...
fun ApplicationCall.user(): User {
    ...
    return User(
        id = principal.subject ?: throw AppError.api(
        ...
        username = username,
        entitlements =
            principal.payload.getClaim(AuthConfiguration.entitlementsClaim)
...
fun Application.installJwtAuth(
    ...
    val jwkProvider =
        JwkProviderBuilder(URI(jwksEndpoint).toURL())
    ...
    install(Authentication) {
        jwt("admin") {
            ...
            validate { credential ->
                val entitlements =
                    credential.payload.getClaim(entitlementsClaim).asList(String::class.java)
