...
import net.barrage.llmao.core.workflow.OutgoingSystemMessage.WorkflowClosed
import net.barrage.llmao.core.workflow.OutgoingSystemMessage.WorkflowOpen
...
class SessionManager {
  ...
  suspend fun handleMessage(session: Session, message: String, emitter: Emitter) {
    try {
      ...
      throw e as? AppError ?: AppError.internal("Error in workflow", original = e)
    }
  }
  ...
  private suspend fun handleMessage(
    ...
    when (message) {
      is IncomingSystemMessage.CreateNewWorkflow -> {
        ...
        emitters[session]?.emit(WorkflowOpen(workflow.id), OutgoingSystemMessage.serializer())
      ...
      is IncomingSystemMessage.CloseWorkflow -> {
        workflows.remove(session)?.let {
          emitters[session]?.emit(WorkflowClosed(it.id), OutgoingSystemMessage.serializer())
          it.cancelStream()
      ...
      is IncomingSystemMessage.WorkflowInput ->
        try {
          ...
          throw e as? AppError ?: AppError.internal("Unexpected error in workflow", original = e)
        }
    }
  }
}
