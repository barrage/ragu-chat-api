...
class Tools(
    ...
    suspend fun processToolCall(data: ToolCallData): String {
        ...
        if (data.id == null) {
            LOG.debug(
                "Tool call '{}' - ID is null, result will not be correlated with call",
                data.name
            )
        }
...
sealed class ToolEvent {
    ...
    data class ToolResult(val result: String) : ToolEvent()
}
 ...
 */
@Serializable
data class ToolDefinition(val type: String, val function: ToolFunctionDefinition)
