package net.barrage.llmao.core.impl.llm.openai
...
class OpenAI(endpoint: String, apiKey: String, private val models: List<String>) :
    ...
    override suspend fun completionStream(
        ...
        return this.client.chatCompletions(chatRequest)
            .map { chunk -> chunk.toNativeMessageChunk() }
    }
                    ...
                    OpenAiChatMessage.User(
                        (content as ContentMulti).content.map {
                            when (it) {
                                ...
                                is ChatMessageContentPart.Image -> ImagePart(
                                    it.imageUrl.url,
                                    it.imageUrl.detail
                                )
                            }
        ...
        "tool" -> {
            ...
            val callId =
                toolCallId
                    ?: throw AppError.api(
                        ErrorReason.InvalidParameter,
                        "Tool message must have tool call id"
                    )
...
fun OpenAiFinishReason.toNativeFinishReason(): FinishReason {
    return when (this) {
        ...
        OpenAiFinishReason.ContentFilter -> FinishReason.ContentFilter
            ...
            throw AppError.api(
                ErrorReason.InvalidParameter,
                "Unrecognized Open AI finish reason '$this'"
            )
    }
}
