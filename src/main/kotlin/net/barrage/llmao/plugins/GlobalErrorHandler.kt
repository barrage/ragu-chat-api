        ...
        exception<Throwable> { call, throwable ->
            ...
            when (throwable) {
                ...
                is RequestValidationException -> {
                    ...
                    throwable.reasons.forEach {
                        errors.add(apiError("Validation error", it))
                    }
                }
                ...
                is BadRequestException -> {
                    ...
                    if (throwable.cause != null) {
                        ...
                        if (cause is JsonConvertException) {
                                ...
                                if (cause.localizedMessage.contains("UUID")) {
                                    ...
                                    apiError(
                                        ...
                                        cause.localizedMessage.split("\n")[0]
                                            .split(": ")
                                            .subList(2, 4).joinToString { it }
                                    )
                                )
                            } else if (throwable.localizedMessage.startsWith("Failed to convert request body to class")) {
                                val className = throwable.localizedMessage.split(" ")[7]
                                val clazz = Class.forName(className).kotlin as KClass<*>
                                errors.add(
                                    apiError(
                                        "Bad request",
                                        "Expected request body: " + printClassDefinition(clazz)
                                    )
                                )
                            } else {
                                errors.add(
                            ...
                            errors.add(apiError("Bad request", "Invalid number format ($numberClass) for input $input"))
