...
class Weaver(config: WeaviateConfig) {
  ...
  fun deleteVectors(documentId: String, collection: String): BatchDeleteResponse {
    ...
    val result =
      client.batch().objectsBatchDeleter().withClassName(collection).withWhere(whereFilter).run()
  ...
  fun countItems(collection: String): Double? {
    val countField =
      Field.builder().name("meta").fields(Field.builder().name("count").build()).build()

    val result = client.graphQL().aggregate().withClassName(collection).withFields(countField).run()
  ...
  fun createCollection(clazz: WeaviateClass): WeaviateClass {
    val weaviateClass: WeaviateClass = WeaviateClass.builder().className(clazz.className).build()
  ...
  fun query(embeddings: List<Int>, options: VectorQueryOptions): List<DocumentChunk> {
    val fields =
      options.fields.split(",").map { fieldName -> Field.builder().name(fieldName.trim()).build() }
      ...
      client
        ...
        .withNearVector(
          NearVectorArgument.builder()
            ...
            .distance(options.distanceFilter?.toFloat())
            .build()
        )
    ...
    val documentChunks =
      when (val collection = get?.get(options.collection) as? List<*>) {
        is List<*> ->
    ...
    for (i in embeddings.indices) {
      ...
      val obj =
        WeaviateObject.builder()
          ...
          .properties(mapOf("content" to chunk.content, "documentId" to documentId) + metadataMap)
          .build()
