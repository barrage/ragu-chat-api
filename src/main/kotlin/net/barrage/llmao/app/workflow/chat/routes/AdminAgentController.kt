...
import net.barrage.llmao.app.workflow.chat.model.SearchFiltersAdminAgentsQuery
...
fun Route.adminAgentsRoutes(agentService: AdminAgentService, settings: Settings) {
    ...
    route("/admin/agents") {
        get(adminGetAllAgents()) {
            ...
            val filters =
                call.query(SearchFiltersAdminAgentsQuery::class).toSearchFiltersAdminAgents()
            val activeWappAgentId = settings.get(WhatsappAgentId.KEY)?.let { tryUuid(it) }
            val agents =
                agentService.listAgents(pagination, filters)
                    .map { it.toAgentDisplay(activeWappAgentId) }
            ...
            route("/versions") {
                ...
                route("/{versionId}") {
                    get(getAgentConfigurationWithEvaluationCounts()) {
                        ...
                        val version = agentService.getAgentConfigurationWithEvaluationCounts(
                            agentId,
                            versionId
                        )
        ...
        post("/import", importAgents()) {
            ...
            if (agents == null) {
                throw AppError.api(
                    ErrorReason.InvalidParameter,
                    "Missing JSON file on field 'agents'"
                )
            }
...
private fun getAgentVersions(): RouteConfig.() -> Unit = {
    ...
    response {
        HttpStatusCode.OK to
                {
                    description =
                        "A list of AgentConfiguration objects representing all the agent versions"
