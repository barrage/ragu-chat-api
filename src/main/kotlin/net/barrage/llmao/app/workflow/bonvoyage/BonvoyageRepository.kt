...
class BonvoyageRepository(override val dslContext: DSLContext) : Atomic, MessageRepository {
        ...
        return dslContext
            ...
            .where(
                userId?.let { BONVOYAGE_TRAVEL_REQUESTS.USER_ID.eq(userId) }
                    ?: DSL.noCondition()
                        .and(
                            status?.let { BONVOYAGE_TRAVEL_REQUESTS.STATUS.eq(status.name) }
                                ?: DSL.noCondition()
                        )
            )
            .asFlow()
        ...
        return dslContext
            ...
            .where(
                BONVOYAGE_TRAVEL_REQUESTS.ID.eq(id)
                    .and(userId?.let { BONVOYAGE_TRAVEL_REQUESTS.USER_ID.eq(userId) }
                        ?: DSL.noCondition())
            )
            .awaitFirstOrNull()
            ?.into(BONVOYAGE_TRAVEL_REQUESTS)
    ...
    suspend fun getTripAggregate(id: KUUID, userId: String? = null): BonvoyageTripExpenseAggregate {
        val trip =
            dslContext
                ...
                .where(
                    BONVOYAGE_TRIPS.ID.eq(id)
                        .and(userId?.let { BONVOYAGE_TRIPS.USER_ID.eq(userId) }
                            ?: DSL.noCondition())
                )
                .awaitFirstOrNull()
                ?.into(BONVOYAGE_TRIPS)
    ...
    suspend fun updateExpense(
        ...
        return dslContext
            ...
            .returning()
            .awaitFirstOrNull()
            ?.toTravelExpense() ?: throw AppError.api(
