...
import io.ktor.server.routing.route
import net.barrage.llmao.app.workflow.bonvoyage.ApproveTravelRequest
import net.barrage.llmao.app.workflow.bonvoyage.BonvoyageAdminApi
import net.barrage.llmao.app.workflow.bonvoyage.BonvoyageTravelManager
import net.barrage.llmao.app.workflow.bonvoyage.BonvoyageTravelManagerUserMapping
...
import net.barrage.llmao.app.workflow.bonvoyage.BonvoyageUser
import net.barrage.llmao.app.workflow.bonvoyage.BulkInsertTrip
import net.barrage.llmao.app.workflow.bonvoyage.InsertTrip
import net.barrage.llmao.app.workflow.bonvoyage.TravelManagerUserMappingInsert
import net.barrage.llmao.app.workflow.bonvoyage.TravelRequestStatusUpdate
                ...
                post(updateTravelRequestStatus()) {
                    ...
                    when (request.status) {
                        BonvoyageTravelRequestStatus.APPROVED -> {
                            ...
                            val trip =
                                api.approveTravelRequest(call.user().toBonvoyageUser(), approval)
                            call.respond(trip)
                        }
...
private fun updateTravelRequestStatus(): RouteConfig.() -> Unit = {
    ...
    response {
        ...
        HttpStatusCode.InternalServerError to
                {
                    description =
                        "Internal server error occurred while updating travel request status"
                    body<List<AppError>> {}
                }
    }
}
...
private fun createManagerUserMapping(): RouteConfig.() -> Unit = {
    ...
    response {
        ...
        HttpStatusCode.InternalServerError to
                {
                    description =
                        "Internal server error occurred while creating travel manager user mapping"
                    body<List<AppError>> {}
                }
    }
}
...
private fun deleteManagerUserMapping(): RouteConfig.() -> Unit = {
    ...
    response {
        ...
        HttpStatusCode.InternalServerError to
                {
                    description =
                        "Internal server error occurred while deleting travel manager user mapping"
                    body<List<AppError>> {}
                }
    }
}
