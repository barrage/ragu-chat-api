...
class AgentRepository(private val dslContext: DSLContext) {
    ...
    suspend fun getAllAdmin(
        ...
        val total =
            dslContext.selectCount().from(AGENTS).where(conditions).awaitSingle().value1() ?: 0
        ...
        return dslContext.transactionCoroutine { tx ->
            ...
            val config =
                context
                    ...
                    .set(
                        AGENT_CONFIGURATIONS.PRESENCE_PENALTY,
                        create.configuration.presencePenalty
                    )
                    .set(
                        AGENT_CONFIGURATIONS.TITLE_INSTRUCTION,
                    ...
                    .set(
                        AGENT_CONFIGURATIONS.ERROR_MESSAGE,
                        create.configuration.instructions?.errorMessage
                    )
                    .returning()
                    .awaitSingle()
    ...
    suspend fun getAgentCounts(): AgentCounts {
        ...
        val active: Int =
            dslContext.selectCount().from(AGENTS).where(AGENTS.ACTIVE.isTrue).awaitSingle().value1()
                ?: 0
    ...
    suspend fun updateAvatar(id: KUUID, avatarPath: String) {
        dslContext.update(AGENTS).set(AGENTS.AVATAR, avatarPath).where(AGENTS.ID.eq(id))
            .awaitSingle()
    }
            ...
            add?.let { additions ->
                ...
                try {
                    tx
                        ...
                        .set(
                            AGENT_COLLECTIONS.MAX_DISTANCE,
                            excluded(AGENT_COLLECTIONS.MAX_DISTANCE)
                        )
                        .awaitLast()
    ...
    suspend fun getAgentConfiguration(agentId: KUUID, versionId: KUUID): AgentConfiguration {
        return dslContext
            ...
            .from(AGENT_CONFIGURATIONS)
            .where(
                AGENT_CONFIGURATIONS.AGENT_ID.eq(agentId).and(AGENT_CONFIGURATIONS.ID.eq(versionId))
            )
            .awaitSingle()
            ?.into(AGENT_CONFIGURATIONS)
            ?.toAgentConfiguration()
            ...
            groups.remove?.let { remove ->
                for (group in remove) {
                    context
                        ...
                        .where(
                            AGENT_PERMISSIONS.AGENT_ID.eq(agentId)
                                .and(AGENT_PERMISSIONS.GROUP.eq(group))
                        )
                        .awaitSingle()
                }
            }
