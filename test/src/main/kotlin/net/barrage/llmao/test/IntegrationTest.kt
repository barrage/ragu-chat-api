...
open class IntegrationTest(
  ...
  private val log = logger(IntegrationTest::class)
  ...
  init {
    ...
    // Feature adapters
    cfg = cfg.mergeWith(MapApplicationConfig("ktor.features.whatsApp" to enableWhatsApp.toString()))
  ...
  private fun loadWiremock() {
    ...
    log.info("Loaded {} wiremock mappings", wiremock!!.listAllStubMappings().mappings.size)

    wiremock!!.start()
    val url = wiremock!!.baseUrl()

    log.info("Wiremock started on $url")
