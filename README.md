...
Client messages are sent either as `system` or `chat` messages. System messages are used to send system commands to the
server, while chat messages are used to send chat messages to the server. Chat messages can only be sent after opening a
chat.
...
System events occur outside of a chat. These events are sent to all connected clients regardless of whether they are
participating in a chat.

- `agent_deactivated`

Sent when an agent is deactivated by an administrator. It contains the id of the deactivated agent.
